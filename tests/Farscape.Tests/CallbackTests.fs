...
module CallbackSerializerTests =
    ...
    let ``callback registrations round-trip through TOML`` () =
        let project : PilotProject = {
            Library = { Name = "gtk"; Headers = ["gtk.h"]; XmlProtocols = []; IncludePaths = []; Defines = []; TransitiveHeaders = []; MacroPrefixes = [] }
    ...
    let ``listener structs round-trip through TOML`` () =
        let project : PilotProject = {
            Library = { Name = "wayland"; Headers = ["wayland-client.h"]; XmlProtocols = []; IncludePaths = []; Defines = []; TransitiveHeaders = []; MacroPrefixes = [] }
            Output = { Mode = "fidelity"; Directory = "./out" }
            Namespaces = []
            ErrorConventions = None
            Options = None
