...
- **PilotTypes.fs**: `ErrorConvention` DU (Errno/ReturnCode/NoErrorConvention), `ErrorConventionSpec` with default + per-function overrides, added to `PilotProject`
- **PilotSerializer.fs**: TOML `[error_conventions]` section serialize/deserialize
...
## Key APIs
- `ErrnoModuleGenerator.generate macros namespace library` → `string option` (rendered Clef source)
