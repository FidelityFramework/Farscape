...
Farscape implements a **two-layer binding model**. Layer 1 generates `[<FidelityExtern>]` stubs with `Unchecked.defaultof<T>` bodies that feed the FNCS/Baker/Alex pipeline. Layer 2 generates idiomatic F# wrappers that call Layer 1 and add error handling, type safety, and null checking. Both layers are implemented and working.
...
Both layers flow through the same pipeline: FNCS → PSG → Baker → Alex → MLIR → LLVM → native binary. The wrapper layer compiles to zero overhead: Result types are erased, comparisons inline, the raw stub call remains. There is no P/Invoke in the Fidelity framework; these wrappers target the native pipeline exclusively.
