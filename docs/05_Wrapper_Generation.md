# Layer 2: Idiomatic F# Wrapper Generation

Farscape implements a **two-layer binding model**. Layer 1 generates raw `Unchecked.defaultof<T>` stubs that Alex replaces with MLIR. Layer 2 generates idiomatic F# wrappers that call Layer 1 and add error handling, type safety, and null checking.

## Two-Layer Architecture

```
Layer 1 — Platform.Bindings (raw extern stubs):
...
Both layers flow through the same pipeline: PSG → Baker → Alex → MLIR → LLVM → native binary. The wrapper layer compiles to zero overhead — Result types are erased, comparisons inline, the raw stub call remains.
...
```bash
# Layer 1 only (existing behavior, unchanged)
farscape generate -h /usr/include/stdio.h -l libc -m fidelity -o ./output

# Layer 1 + Layer 2 wrappers
farscape generate -h /usr/include/stdio.h -l libc -m fidelity-wrappers -o ./output

# Project-based with wrappers
farscape project --project libc.moya.toml --wrappers
```
