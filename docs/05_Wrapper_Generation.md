...
Farscape implements a **two-layer binding model**. Layer 1 generates `[<FidelityExtern>]` binding declarations with `NativeDefault.zeroed ()` bodies that feed the CCS/Baker/Alex pipeline. Layer 2 generates idiomatic Clef wrappers that call Layer 1 and add error handling, type safety, and null checking. Both layers are implemented and working.
