...
5. `[callbacks]` registrations (docs/07) and the Tier A closure design (docs/10, userdata plus destroy hook) cover `g_signal_connect_data`'s lifetime plumbing; the handler's type comes from the header's callback typedef, and `GCallback` is `void (*)(void)`. The entry signature of a signal lives in GObject's introspection data, so `[library] introspection` and `[[namespace]] signals` (docs/07) now read it: `IntrospectionParser` resolves each selected signal's parameters and result to C spellings, and `TypedSignalGenerator` emits, per signal, a listener record, its `CallbackDescriptor` and a connect over `g_signal_connect_data` in `Bridge/Signals.clef`, in the Tier C form. The desktop binding connects its signals through that bridge; WrenHello supplies only a domain handler.

The former claim that a function pointer has no absence representation was incorrect. `ffi-boundary.md` §5.5 requires `Option<FnPtr<F>>` for nullable callback slots, while the September compiler rejects such extern arguments. This discrepancy is distinct from the settled rule that interior handles and callback entries never admit null. `docs/08_Nullable_Pointer_Architecture.md` records the current conversion contract and the separate default-nullability and option-layout documentation conflicts.

The landing repairs add nonnull callback carriers, per-entry native descriptors, descriptor-bound release notifications, non-C-identifier signal identities, nullable-result rejection, floating-entry rejection, namespace-qualified selection with ambiguity diagnostics, and restored scalar/enum/nonvoid tests. The documented inline-table callback syntax is now read by the pilot parser. Direct callback parameters in regenerated Layer 2 APIs accept declared entry records; the underlying private extern retains the complete C arity.

The native listener signature remains Layer 2 plumbing. Passing its user-data parameters or native string ownership to application code does not satisfy the public boundary rule. `ClosedCallbackDescriptor` now establishes a binding-owned factory and conversion adapter for immutable closed module handlers, with the native ABI still required from `CallbackDescriptor`. Generated signal `on` helpers hide native context; the desktop overlay also converts message payloads and owns their release. Full captured-environment retention and release remain the separate Composer C-01 §§6.7 and 10.8 contract.

Remaining independent gaps:

- A signal connects undetailed; the `signal::detail` form is not projected.
- Callback auto-discovery under the legacy wrapper path skips `g_signal_connect_data` because it has two function-pointer parameters. Typed signals use their explicit introspection contract instead.
- With clang 22 the return type of `strlen` in `string.h` is read as `__size_t` from the function type string and does not resolve; `size_t` parameters resolve. The desktop binding uses length-preserving `GBytes` transfer and never infers message length from a C string terminator.
- Declarations are kept only from files under the include root chosen for each header. GLib's generated `glibconfig.h` lives under `/usr/lib/glib-2.0/include` and cannot be listed directly; listing `/usr` first retains the needed declarations. The WREN pilots do this.
